...
export default async function AdminDashboard() {
  const [totalRooms, availableRooms, occupiedRooms, pendingBookings] =
    await Promise.all([
      prisma.room.count(),
  ...
  return (
    <div className="space-y-8 animate-fade-in">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        ...
        <div className="bg-surface p-6 rounded-2xl shadow-sm border border-border">
          <h3 className="text-amber-600/80 text-sm font-medium uppercase tracking-wider">Booking Pending</h3>
          <div className="mt-4 flex items-baseline gap-2">
            <span className="text-4xl font-bold text-amber-600">{pendingBookings}</span>
